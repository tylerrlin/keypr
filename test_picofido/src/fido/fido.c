...
int scan_files_fido()
{
    ...
    if (ef_keydev)
    {
        // Check if keydev needs (re)generation: either empty, or existing data
        // fails to decrypt (e.g. previously PIN-encrypted, PIN now removed).
        uint8_t _probe[32] = {0};
        bool need_keygen = (!file_has_data(ef_keydev) && !file_has_data(ef_keydev_enc));
        if (!need_keygen && file_has_data(ef_keydev) && load_keydev(_probe) != PICOKEY_OK)
        {
            printf("KEY DEVICE decrypt failed. Regenerating...\n");
            // Wipe old keydev so we regenerate cleanly
            file_put_data(ef_keydev, NULL, 0);
            need_keygen = true;
        }
        mbedtls_platform_zeroize(_probe, sizeof(_probe));
        if (need_keygen)
        {
