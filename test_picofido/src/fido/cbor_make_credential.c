...
int cbor_make_credential(const uint8_t *data, size_t len)
{
    ...
    if (options.present)
    {
        // uv=true: we advertise uv=true in getInfo (button press = UV).
        // Accept uv=true instead of returning INVALID_OPTION; the UV flag
        // will be set in the authenticatorData after user presence is confirmed.
        if (options.up != NULL)
        { // 5.6
    ...
    // We advertise uv=true in getInfo (built-in UV via button press), so Chrome
    // relies on the device performing UV without a PIN token. Never require PUAT
    // here — user presence check later will satisfy UV.
    // Original: if (pinUvAuthParam.present == false && options.uv == pfalse && file_has_data(ef_pin))
    ...
    for (size_t e = 0; e < excludeList_len; e++)
    { // 12.1
    ...
    if (options.up == ptrue || options.up == NULL)
    { // 14.1
        if (pinUvAuthParam.present == true)
        {
    ...
    *pa++ = flags;
    ...
    if (options.rk == ptrue)
    {
        ...
        memcpy(pa, cred_idr, sizeof(cred_idr));
        pa += sizeof(cred_idr);
    }
    else
    {
        pa += put_uint16_t_be(cred_id_len, pa);
        memcpy(pa, cred_id, cred_id_len);
        pa += (uint16_t)cred_id_len;
    }
    memcpy(pa, cbor_buf, rs);
    pa += (uint16_t)rs;
    memcpy(pa, ext, ext_len);
    pa += (uint16_t)ext_len;
