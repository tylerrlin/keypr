...
// ─── NOTE ON CBOR
// ───────────────────────────────────────────────────────────── For a
// production build, use tinycbor or cn-cbor. For the hackathon, this implements
// minimal hand-rolled CBOR parsing/encoding sufficient for the CTAP2 commands
// we need. A proper CBOR lib is strongly recommended for production.
...
static size_t cbor_skip(const uint8_t *buf, size_t max)
{
    ...
    if (ai == 24) {
        ...
        extra = ((uint64_t)buf[1] << 8) | buf[2];
        hdr = 3;
    } else if (ai == 26) {
        extra = ((uint64_t)buf[1] << 24) | ((uint64_t)buf[2] << 16) |
    ...
    case 0:
    case 1:
        return hdr; // uint, negint
    case 2:
    case 3:
        return hdr + (size_t)extra; // bytes, text
    case 4: {                       // array
        size_t total = hdr;
        for (uint64_t i = 0; i < extra; i++)
            total += cbor_skip(buf + total, max - total);
        return total;
    }
    case 5: { // map
        size_t total = hdr;
        for (uint64_t i = 0; i < extra * 2; i++)
            total += cbor_skip(buf + total, max - total);
        return total;
    }
    default:
...
static const uint8_t *cbor_read_bytes(const uint8_t *buf, size_t *out_len,
                                      size_t *consumed)
{
    uint8_t ai = buf[0] & 0x1F;
    size_t hdr = 1;
...
static const uint8_t *cbor_read_text(const uint8_t *buf, size_t *out_len,
                                     size_t *consumed)
{
    return cbor_read_bytes(buf, out_len,
                           consumed); // same encoding, different major type
}
...
uint8_t ctap2_get_info(uint8_t *out_buf, size_t *out_len)
{
    ...
    *p++ = 0xA3; // map(3)
    p += cbor_write_text(p, "rk");
    *p++ = 0xF5; // true
    p += cbor_write_text(p, "up");
    *p++ = 0xF5; // true
    p += cbor_write_text(p, "uv");
    *p++ = 0xF4; // false
...
// ─── makeCredential
// ───────────────────────────────────────────────────────────
uint8_t ctap2_make_credential(const uint8_t *cbor, size_t len, uint8_t *out_buf,
                              size_t *out_len)
{
        ...
        case 2: { // rp { id: "...", name: "..." }
            if ((cbor[pos] & 0xE0) == 0xA0) {
                ...
                for (int j = 0; j < rp_map_len; j++) {
                    size_t key_len;
                    const uint8_t *k =
                        cbor_read_text(cbor + pos, &key_len, &consumed);
                    pos += consumed;
                    if (key_len == 2 && memcmp(k, "id", 2) == 0) {
                        rp_id =
                            cbor_read_text(cbor + pos, &rp_id_len, &consumed);
                        pos += consumed;
                    } else {
        ...
        case 3: { // user { id: bytes, name: "..." }
            if ((cbor[pos] & 0xE0) == 0xA0) {
                ...
                for (int j = 0; j < u_map_len; j++) {
                    ...
                    pos += consumed;
                    if (key_len == 2 && memcmp(k, "id", 2) == 0) {
                        user_id = cbor_read_bytes(cbor + pos, &user_id_len,
                                                  &consumed);
                        pos += consumed;
                    } else if (key_len == 4 && memcmp(k, "name", 4) == 0) {
                        user_name = (const char *)cbor_read_text(
                            cbor + pos, &user_name_len, &consumed);
                        pos += consumed;
                    } else {
                        pos += cbor_skip(cbor + pos, len - pos);
                    }
    ...
    if (!crypto_random(cred.id, CREDENTIAL_ID_LEN))
        return CTAP2_ERR_OTHER;
    if (!crypto_generate_keypair(cred.private_key, pub_x, pub_y))
        return CTAP2_ERR_OTHER;
    if (!crypto_sha256(rp_id, rp_id_len, cred.rp_id_hash))
        return CTAP2_ERR_OTHER;
    ...
    if (!storage_store_credential(&cred))
        return CTAP2_ERR_KEY_STORE_FULL;

    // Build authenticator data
    uint8_t auth_data[512];
    size_t auth_data_len =
        crypto_build_auth_data(cred.rp_id_hash, true, false, true, 0, cred.id,
                               pub_x, pub_y, auth_data, sizeof(auth_data));
    ...
    uint8_t sig[72];
    size_t sig_len = 0;
    if (!crypto_sign(cred.private_key, to_sign, auth_data_len + 32, sig,
                     &sig_len)) {
        return CTAP2_ERR_OTHER;
    }
...
// ─── getAssertion
// ─────────────────────────────────────────────────────────────
uint8_t ctap2_get_assertion(const uint8_t *cbor, size_t len, uint8_t *out_buf,
                            size_t *out_len)
{
    ...
    char buf[32] = {0};
    status = ble_wait_for_response(buf, 5000);

    if (!status || buf[0] == 0x41) {
        return CTAP2_ERR_OPERATION_DENIED;
    }
                    ...
                    if ((cbor[pos] & 0xE0) == 0xA0) {
                        ...
                        for (int k = 0; k < entry_map_len; k++) {
                            ...
                            if (key_len == 2 && memcmp(mk, "id", 2) == 0) {
                                allow_credential_id = cbor_read_bytes(
                                    cbor + pos, &allow_cred_id_len, &consumed);
                                pos += consumed;
                            } else {
                                pos += cbor_skip(cbor + pos, len - pos);
                            }
    ...
    if (!crypto_sha256(rp_id, rp_id_len, rp_id_hash))
        return CTAP2_ERR_OTHER;
    ...
    if (!found) {
        Serial.printf(TAG, "No credential found for RP: %.*s", (int)rp_id_len,
                      rp_id);
    ...
    size_t auth_data_len =
        crypto_build_auth_data(rp_id_hash, true, false, false, cred.sign_count,
                               NULL, NULL, NULL, auth_data, sizeof(auth_data));
    ...
    *out_len = p - out_buf;
    Serial.printf("getAssertion success, sign_count=%lu",
...
// ─── Dispatcher
// ───────────────────────────────────────────────────────────────
uint8_t ctap2_process(const uint8_t *in_buf, size_t in_len, uint8_t *out_buf,
                      size_t *out_len)
{
    if (in_len == 0) {
