...
bool crypto_sha256(const uint8_t *data, size_t len, uint8_t *hash_out) {
    // Use _ret variants - works on both old and new mbedTLS under Arduino ESP32
    mbedtls_sha256_context ctx;
    mbedtls_sha256_init(&ctx);
    mbedtls_sha256_starts_ret(&ctx, 0);
    mbedtls_sha256_update_ret(&ctx, data, len);
...
bool crypto_generate_keypair(uint8_t *private_key, uint8_t *pub_x, uint8_t *pub_y) {
    ...
    int ret = mbedtls_ecdsa_genkey(&ctx, MBEDTLS_ECP_DP_SECP256R1,
                                   mbedtls_ctr_drbg_random, &ctr_drbg);
    if (ret != 0) {
        Serial.printf("[CRYPTO] genkey failed: -0x%04x\n", -ret);
        mbedtls_ecdsa_free(&ctx);
        return false;
    }

    // Direct member access - Arduino ESP32 mbedTLS exposes struct members
    size_t privlen = mbedtls_mpi_size(&ctx.d);
    memset(private_key, 0, PRIVATE_KEY_LEN);
    mbedtls_mpi_write_binary(&ctx.d, private_key + (PRIVATE_KEY_LEN - privlen), privlen);
    ...
    mbedtls_ecdsa_free(&ctx);
    Serial.println("[CRYPTO] keypair generated OK");
    return true;
}
...
bool crypto_sign(const uint8_t *private_key, const uint8_t *data, size_t data_len,
    ...
    // Old mbedTLS API: no max_sig_len param, sig_len is size_t*
    int ret = mbedtls_ecdsa_write_signature(
        &ctx, MBEDTLS_MD_SHA256,
        hash, sizeof(hash),
        sig_buf, sig_len,
    ...
    if (ret != 0) {
        Serial.printf("[CRYPTO] sign failed: -0x%04x\n", -ret);
        return false;
    }
    Serial.printf("[CRYPTO] sign OK, sig_len=%d\n", *sig_len);
    return true;
}
    ...
    size_t out_buf_max)
{
    ...
    // flags
    uint8_t flags = 0;
    if (user_present)          flags |= 0x01;  // UP
    if (user_verified)         flags |= 0x04;  // UV
    if (include_attested_cred) flags |= 0x40;  // AT
    out_buf[offset++] = flags;
    ...
    if (include_attested_cred && credential_id && pub_key_x && pub_key_y) {
        ...
        // 3: alg = -7 (ES256)
        out_buf[offset++] = 0x03;
        out_buf[offset++] = 0x26;  // -7 in CBOR

        // -1: crv = 1 (P-256)
        out_buf[offset++] = 0x20;  // -1 in CBOR
        out_buf[offset++] = 0x01;

        // -2: x (32 bytes)
        out_buf[offset++] = 0x21;  // -2 in CBOR
        out_buf[offset++] = 0x58;
        out_buf[offset++] = 0x20;
        memcpy(out_buf + offset, pub_key_x, 32);
        offset += 32;

        // -3: y (32 bytes)
        out_buf[offset++] = 0x22;  // -3 in CBOR
        out_buf[offset++] = 0x58;
        out_buf[offset++] = 0x20;
        memcpy(out_buf + offset, pub_key_y, 32);
        offset += 32;
    }

    Serial.printf("[CRYPTO] auth_data built, len=%d\n", offset);
    return offset;
}